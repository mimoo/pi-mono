/**
 * OpenAI GPT-5.2 Codex with image input + weather web search.
 *
 * 1) Describes a hardcoded image file.
 * 2) Searches for New York weather for the next two weeks.
 *
 * Uses OPENAI_API_KEY from environment, with a fallback that reads `.env`
...
function parseOpenAiApiKey(envContent: string): string | undefined {
	for (const line of envContent.split(/\r?\n/)) {
		const trimmed = line.trim();
		if (trimmed.length === 0 || trimmed.startsWith("#")) continue;

		const match = trimmed.match(/^(?:export\s+)?OPENAI_API_KEY\s*=\s*(.+)$/);
		if (!match) continue;
...
function getOpenAiApiKey(): string {
	...
	for (const envPath of envPaths) {
		...
		const parsed = parseOpenAiApiKey(content);
		if (parsed) {
			process.env.OPENAI_API_KEY = parsed;
			return parsed;
		}
	}

	throw new Error("OPENAI_API_KEY was not found. Set it in your environment or in a .env file (OPENAI_API_KEY=...).");
}
